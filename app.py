...
def load_model_scaler():
    try:
        ...
        st.error(f"Error loading model files: {e}. Make sure model.pkl and scaler.pkl are in the correct directory.")
        st.stop()
...
def load_data():
    try:
        ...
        st.error(f"Error loading data file: {e}. Make sure cleaned.csv is in the correct directory.")
        st.stop()
    except Exception as e:
...
with tab1:
    ...
    avg_price    = df['avg_price_per_room'].mean() if not df['avg_price_per_room'].isna().all() else 0
    avg_lead     = df['lead_time'].mean() if not df['lead_time'].isna().all() else 0
...
with tab2:
    ...
    if predict_btn:
        try:
            ...
            # Map probabilities to class labels robustly
            ...
            if cancel_class in classes and not_cancel_class in classes:
                ...
                no_cancel_prob = float(proba[1]) * 100 if len(proba) > 1 else 0
...
with tab3:
    if total > 0:
        ...
        # Monthly trends
        if 'date of arrival' in df.columns and not df['date of arrival'].isna().all():
            st.markdown('<div class="section-header">Monthly Arrival Trends</div>', unsafe_allow_html=True)
            df_m = df.dropna(subset=['date of arrival']).copy()
            if len(df_m) > 0:
